...
export class Coords {
  ...
  static intersectionLLCoord (line1: Line, line2: Line): Coords {
    try {
  ...
  static intersectionSCCoord (L: Segment, C: Circle): Coords {
    try {
      ...
      if (x !== undefined && Coords.distance(A, B) > C.radius) {
 ...
 */
   ...
   */
  static rotationCoord (A: Point | Coords, O: Point | Coords, angle: number): Coords {
    try {
  ...
  /**
     ...
     * @returns [x, y] coordonnées de l'image
     */
  static homothetieCoord (A: Point | Coords, O: Point | Coords, k: number): Coords {
