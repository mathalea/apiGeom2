...
class Grid extends Element2D {
  ...
  draw (): void {
    ...
    if (this.labelX) {
      for (let x = 0; x <= this.figure.xMax; x += this.stepX) {
        this.figure.create('TextByPosition', { text: `$${x.toString()}$`, fontSize: '8pt', x, y: 0, isChild: true, isSelectable: true, anchor: 'topCenter', dyInPixels: 3 })
      }
    ...
    if (this.labelY) {
      for (let y = 0; y <= this.figure.yMax; y += this.stepY) {
        this.figure.create('TextByPosition', { text: `$${y.toString()}$`, fontSize: '8pt', x: 0, y, isChild: true, isSelectable: true, anchor: 'middleRight', dxInPixels: -10, dyInPixels: 2 })
      }
      for (let y = 0; y >= this.figure.yMin; y -= this.stepY) {
        this.figure.create('TextByPosition', { text: `$${y.toString()}$`, fontSize: '8pt', x: 0, y, isChild: true, isSelectable: true, anchor: 'middleRight', dxInPixels: -10, dyInPixels: 2 })
      }
